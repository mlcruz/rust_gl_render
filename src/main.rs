...
fn main() {
    ...
    let mut g_camera_theta = 0.0; // Ângulo no plano ZX em relação ao eixo Z
    let mut g_camera_phi = 0.0; // Ângulo em relação ao eixo Y
    let mut g_camera_distance = 2.5; // Distância da câmera para a origem
    ...
    let mut camera_position_c = glm::vec4(0.0, 0.0, 0.0, 0.0); // Ponto "c", centro da câmera
    let mut camera_lookat_l = glm::vec4(0.0, 0.0, 0.0, 0.0); // Ponto "l", para onde a câmera (look-at) estará sempre olhando
    let mut camera_view_vector = camera_lookat_l - camera_position_c; // Vetor "view", sentido para onde a câmera está virada
    let mut camera_up_vector = glm::vec4(0.0, 0.0, 0.0, 0.0); // Vetor "up" fixado para apontar para o "céu" (eito Y global)
    ...
    let update_camera = |x: &mut f32,
                         y: &mut f32,
                         z: &mut f32,
                         camera_position_c: &mut glm::Vector4<f32>,
                         camera_view_vector: &mut glm::Vector4<f32>,
                         camera_lookat_l: &mut glm::Vector4<f32>,
                         camera_up_vector: &mut glm::Vector4<f32>| {
        *y = g_camera_distance * sin(g_camera_phi);
        *z = g_camera_distance * cos(g_camera_phi) * cos(g_camera_theta);
        *x = g_camera_distance * cos(g_camera_phi) * sin(g_camera_theta);

        *camera_position_c = glm::vec4(*x, *y, *z, 1.0); // Ponto "c", centro da câmera
        *camera_lookat_l = glm::vec4(0.0, 0.0, 0.0, 1.0); // Ponto "l", para onde a câmera (look-at) estará sempre olhando
        *camera_view_vector = *camera_lookat_l - *camera_position_c; // Vetor "view", sentido para onde a câmera está virada
        *camera_up_vector = glm::vec4(0.0, 1.0, 0.0, 0.0); // Vetor "up" fixado para apontar para o "céu" (eito Y global)
    };
    ...
    unsafe {
        ...
        let view_uniform = gl::GetUniformLocation(program, CString::new("view").unwrap().as_ptr());
        ...
        events_loop.run_forever(|event| {
            ...
            update_camera(
                ...
                &mut y,
                &mut z,
                &mut camera_position_c,
                &mut camera_view_vector,
                &mut camera_lookat_l,
                &mut camera_up_vector,
            );
